...
namespace Reportman.Drawing
{
    ...
    public static class StringUtil
    {
        ...
        public static string SanitizeSubject(string subject)
        {
            ...
            // string sanitizedSubject = Regex.Replace(subject, @"[^\w\s-]", "");
            // No es necesario quitar / o cualquier otro simbolo
            string sanitizedSubject = subject;

            // Limita la longitud del asunto a un valor razonable (por ejemplo, 255 caracteres)
            if (sanitizedSubject.Length > 255)
            {
                sanitizedSubject = sanitizedSubject.Substring(0, 255);
            }
            sanitizedSubject = sanitizedSubject.Replace((char)10, ' ');
    ...
    string hashString)
        ...
        public static bool ContainsAllWords(string input, IEnumerable<string> words)
        {
            ...
            {
                throw new ArgumentNullException("El input y las palabras no pueden ser nulos.");
            }

            // Recorre cada palabra en el enumerable y verifica si está contenida en el input
            return words.All(word => input.IndexOf(word, StringComparison.OrdinalIgnoreCase) >= 0);
        }
        ...
        public static int EAN13CalculateCheckDigit(string codigo)
        {
            ...
            // Validar que el código tenga exactamente 12 dígitos
            if (codigo.Length != 12 || !long.TryParse(codigo, out _))
            {
                throw new ArgumentException("El código debe contener exactamente 12 dígitos.");
            }

            int sumaImpares = 0;
            int sumaPares = 0;

            // Recorrer cada dígito del código
            for (int i = 0; i < 12; i++)
            {
                int digito = int.Parse(codigo[i].ToString());

                // Sumar los dígitos de posición impar y par
                if (i % 2 == 0) // Posición impar (0, 2, 4...)
                    sumaImpares += digito;
                else // Posición par (1, 3, 5...)
                    sumaPares += digito;
            }

            // Calcular la suma total con el peso de los pares
            int sumaTotal = sumaImpares + (sumaPares * 3);

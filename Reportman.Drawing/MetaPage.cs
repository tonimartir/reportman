...
namespace Reportman.Drawing
{
    ...
    public class MetaPage : IDisposable
    {
        ...
        /// <summary>Metafile version format</summary>
		...
		public MetaPage(MetaFile meta)
        {
            ...
            Version = MetaFile.MetaFileVersion.MetaVersion3_0;
        ...
        /// </summary>
        /// <returns>The text referenced in the AnnotationP position</returns>
        public string GetAnnotationText(MetaObject obj)
        {
            if (obj.AnnotationP == 0)
            {
                return null;
            }
            return new string(Pool, obj.AnnotationP - 1, obj.AnnotationS);
        }
        /// <summary>
		...
		public void SaveToStream(Stream astream)
        {
            ...
            {
                Objects[i].SaveToStream(astream, version);
            }
        ...
        public void LoadFromStream(Stream astream)
        {
            ...
            // Mark begin
            ReadBuf(astream, ref buf, 4);
            if (Version != MetaFile.MetaFileVersion.MetaVersion2_2)
            {
                ReadBuf(astream, ref buf, 4);
            ...
            int RECORD_SIZE = MetaObject.GetRECORD_SIZE(version);
            ReadBuf(astream, ref buf, 4);
            int objcount = StreamUtil.ByteArrayToInt(buf, 4);
            if (objcount < 0)
                throw new Exception(Translator.TranslateStr(523));
            buf = new byte[objcount * RECORD_SIZE];
            ReadBuf(astream, ref buf, objcount * RECORD_SIZE);

            Objects.Clear();
            for (i = 0; i < objcount; i++)
            {
                MetaObject obj = MetaObject.CreateFromBuf(buf, i * RECORD_SIZE, version);
                obj.FillFromBuf(buf, i * RECORD_SIZE, version);
                Objects.Add(obj);
            }
            ...
            PrintStepType PrintStep)
        {
            ...
            {
                metaobj.Type1Font = PDFFontType.Embedded;
            }
            else

...
namespace Reportman.Drawing
{
    public class StreamUtil
    {
        ...
        public static void SWriteLine(Stream astream, string astring, bool includeCR = false)
        {
            ...
            if (includeCR)
                astream.Write(CRLFarray, 0, 2);
            else
                astream.Write(LFarray, 0, 1);
            /*
        ...
        /// </param>
        /// <returns>The resulting memory stream</returns>
        static public async System.Threading.Tasks.Task<MemoryStream> StreamToMemoryStreamAsync(Stream astream, int bufsize)
        {
            // Don't increase BUF_SIZE, standerd input
            if (bufsize == 0)
                bufsize = 65535;
            ...
            {
                aresult.Write(buf, 0, readed);
                readed = await astream.ReadAsync(buf, 0, bufsize);
            }
            return aresult;
        }
        /// <summary>
        ...
        /// Generates a MemoryStream from a file
        /// </summary>
        static public async System.Threading.Tasks.Task<MemoryStream> FileToMemoryStreamAsync(string filename)
        {
            MemoryStream rstream = null;
            // Don't increase BUF_SIZE, standerd input
            using (FileStream fstream = new FileStream(filename, FileMode.OpenOrCreate, FileAccess.Read))
        ...
        {
            byte[] buf = new byte[bufSize];
            int readed = from.Read(buf, 0, bufSize);
            while (readed > 0)
            {
        ...
        /// </summary>
        static public void StreamToFile(Stream memstream, string filename, bool overwrite)
        {
            memstream.Seek(0, SeekOrigin.Begin);
            // Don't increase BUF_SIZE, standerd input
            using (FileStream fstream = new FileStream(filename, overwrite ? FileMode.Create : FileMode.CreateNew, FileAccess.Write))
            {
                ...
                int readed = memstream.Read(buffer, 0, BUFFER_SIZE);
                while (readed > 0)
                {
                    fstream.Write(buffer, 0, readed);
                    readed = memstream.Read(buffer, 0, BUFFER_SIZE);
                }
            }
        }
        /// <summary>
        /// Generate a file from a stream
        /// </summary>
        static public async void StreamToFileAsync(Stream memstream, string filename, bool overwrite)
        {
            memstream.Seek(0, SeekOrigin.Begin);
            // Don't increase BUF_SIZE, standerd input
            using (FileStream fstream = new FileStream(filename, overwrite ? FileMode.Create : FileMode.CreateNew, FileAccess.Write))
            {
                const int BUFFER_SIZE = 4096 * 8;
                byte[] buffer = new byte[BUFFER_SIZE];
                int readed = await memstream.ReadAsync(buffer, 0, BUFFER_SIZE);
                while (readed > 0)
                {
                    await fstream.WriteAsync(buffer, 0, readed);
                    readed = await memstream.ReadAsync(buffer, 0, BUFFER_SIZE);
                }
            }
        }
        ...
        static public async System.Threading.Tasks.Task DecompressStreamGZipAsync(Stream origin, Stream dest)
        {
            using (System.IO.Compression.DeflateStream inflater = new System.IO.Compression.DeflateStream(origin, System.IO.Compression.CompressionMode.Decompress))
            {
                ...
                //dest.Capacity = 0;
                byte[] bufuncomp = new byte[bufsize];
                int readed = await inflater.ReadAsync(bufuncomp, 0, bufsize);
                while (readed > 0)
                {
                    await dest.WriteAsync(bufuncomp, 0, readed);
                    readed = await inflater.ReadAsync(bufuncomp, 0, bufsize);
                }
                dest.Seek(0, System.IO.SeekOrigin.Begin);
            }
            ...
            Stream dest)
        {
            using (MemoryStream mstream = new MemoryStream())
            {
                ...
                await DecompressStreamGZipAsync(mstream, dest);
            }
        ...
        /// Dercompress the first memory stream into the second memory stream
        /// </summary>
        static public async System.Threading.Tasks.Task DeCompressBufferAsync(byte[] buffer, int count,
            Stream dest)
        {
            // Check if it's a GZip stream
        ...
        /// </summary>
        static public void CompressStream(Stream memstream,
            MemoryStream dest)
        {
        ...
        /// Compress the first memory stream into the second memory stream
        /// </summary>
        static public async System.Threading.Tasks.Task CompressStreamAsync(MemoryStream memstream,
            MemoryStream dest)
        {
            byte[] bufuncomp = new byte[100000];
            ICSharpCode.SharpZipLib.Zip.Compression.Deflater inf = new ICSharpCode.SharpZipLib.Zip.Compression.Deflater(ICSharpCode.SharpZipLib.Zip.Compression.Deflater.BEST_SPEED);
        ...
        /// Compress the first memory stream into the second memory stream using Gzip
        /// </summary>
        static public async System.Threading.Tasks.Task CompressStreamGZipAsync(MemoryStream memstream,
            MemoryStream dest)
        {
            byte[] bufuncomp = new byte[100000];
            ICSharpCode.SharpZipLib.GZip.GZipOutputStream zstream = new ICSharpCode.SharpZipLib.GZip.GZipOutputStream(dest);
            ...
            Stream dest)
        {
            using (MemoryStream mstream = new MemoryStream())
            {
                ...
                mstream.Seek(0, SeekOrigin.Begin);
                DecompressStreamGZip(mstream, dest);
            }
            dest.Seek(0, SeekOrigin.Begin);
        }
        static public void DecompressStreamGZip(Stream origin, Stream dest)
        {
            using (System.IO.Compression.DeflateStream inflater = new System.IO.Compression.DeflateStream(origin, System.IO.Compression.CompressionMode.Decompress))
            {
                ...
                //dest.Capacity = 0;
                byte[] bufuncomp = new byte[bufsize];
                int readed = inflater.Read(bufuncomp, 0, bufsize);
                while (readed > 0)
                {
                    dest.Write(bufuncomp, 0, readed);
                    readed = inflater.Read(bufuncomp, 0, bufsize);
                }
                dest.Seek(0, System.IO.SeekOrigin.Begin);
            }

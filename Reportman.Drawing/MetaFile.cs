...
namespace Reportman.Drawing
{
    ...
    {
        /// <summary>Smaller, older format supporting not embedded fonts</summary>
        PDF_1_4 = 0,
        /// <summary>Portable modern standard, fonts will be embedded and embedded files can be added</summary>
        PDF_A_3 = 1
    };
    /// <summary>
    ...
    public class EmbeddedFile : ICloneable, IDisposable
    {
        ...
        public string AFRelationShipToString()
        {
            ...
            switch (AFRelationShip)
            {
                case PDFAFRelationShip.Unspecified:
                    Result = "Unspecified";
                    break;
                case PDFAFRelationShip.Alternative:
                    Result = "Alternative";
                    break;
                case PDFAFRelationShip.Data:
                    Result = "Data";
                    break;
                case PDFAFRelationShip.Source:
                    Result = "Source";
                    break;
                case PDFAFRelationShip.Supplement:
                    Result = "Supplement";
                    break;
            }
            return Result;
        ...
        public object Clone()
        {
            ...
            if (Stream != null)
            {
                efile.Stream = new MemoryStream();
                Stream.Seek(0, SeekOrigin.Begin);
                efile.Stream.Write(Stream.GetBuffer(), 0, Convert.ToInt32(Stream.Length));
                Stream.Seek(0, SeekOrigin.Begin);
    ...
    public class MetaFile : IDisposable, ICloneable
    {
        ...
        private byte[] sign3_0 ={(byte)'R',(byte)'P',(byte)'M',(byte)'E',(byte)'T',(byte)'A',
        ...
        public PDFConformanceType PDFConformance
        {
            ...
            set
            {
                if (value == PDFConformanceType.PDF_A_3)
                {
                    if (FVersion < MetaFileVersion.MetaVersion3_0)
                    {
          ...
          string description, string creationDate, string modificationDate, MemoryStream stream)
        {
            ...
            var embededFile = new EmbeddedFile();
            ...
            embededFile.Stream = new MemoryStream();
            stream.Seek(0, SeekOrigin.Begin);
            stream.CopyTo(embededFile.Stream);
            embededFile.Stream.Seek(0, SeekOrigin.Begin);
            embededFile.MimeType = mimeType;
            EmbeddedFiles.Add(embededFile);
        }
		...
		public void Clear()
        {
            FFinished = false;
            Pages.Clear();
            foreach (var embededFile in EmbeddedFiles)
            {
                embededFile.Stream?.Dispose();
            }
            EmbeddedFiles.Clear();
        ...
        private void IntSaveToStream(Stream astream)
        {
            ...
            else
            {
                if (FVersion == MetaFileVersion.MetaVersion3_0)
                {
                    astream.Write(sign3_0, 0, sign3_0.Length);
                }
                else
                {
            ...
            if (FVersion >= MetaFileVersion.MetaVersion3_0)
            {
                byte bytevalue = (byte)PDFConformance;
                bytesValue[0] = bytevalue;
                ...
                astream.Write(bytesValue, 0, 1);
                // Metadata
                WriteStringToStream(DocAuthor, astream);
                WriteStringToStream(DocCreator, astream);
                WriteStringToStream(DocProducer, astream);
                WriteStringToStream(DocCreationDate, astream);
                WriteStringToStream(DocModificationDate, astream);
                WriteStringToStream(DocSubject, astream);
                WriteStringToStream(DocTitle, astream);
                WriteStringToStream(DocKeywords, astream);
                WriteStringToStream(DocXMPContent, astream);
                // Embedded files
                byte[] bufFileCount = StreamUtil.IntToByteArray(EmbeddedFiles.Count);
                astream.Write(bufFileCount, 0, 4);
                foreach (var efile in EmbeddedFiles)
                {
                    Int64 ssize = efile.Stream.Length;
                    WriteStringToStream(efile.FileName, astream);
                    WriteStringToStream(efile.MimeType, astream);
                    WriteStringToStream(efile.Description, astream);
                    WriteStringToStream(efile.ModificationDate, astream);
                    byte[] bufRelation = new byte[1];
                    bufRelation[0] = (byte)efile.AFRelationShip;
                    astream.Write(bufRelation, 0, 1);
                    byte[] bufSize = StreamUtil.Int64ToByteArray(ssize);
        ...
        private void IntLoadFromStream(Stream astream, bool ClearFirst)
        {
            ...
            try
            {
                ...
                if (FVersion >= MetaFileVersion.MetaVersion3_0)
                {
                    ...
                    DocProducer = ReadStringFromStream(astream);
                    DocCreationDate = ReadStringFromStream(astream);
                    DocModificationDate = ReadStringFromStream(astream);
                    DocSubject = ReadStringFromStream(astream);
                    DocTitle = ReadStringFromStream(astream);
                    DocKeywords = ReadStringFromStream(astream);
                    DocXMPContent = ReadStringFromStream(astream);

                    byte[] fileCountArray = new byte[4];
                    // Embedded Files
                    int readed = astream.Read(fileCountArray, 0, 4);
                    if (readed != 4)
                    {
                        throw new Exception("Invalid metafile");
                    }
                    int fileCount = StreamUtil.ByteArrayToInt(fileCountArray, 4);
                    if (fileCount < 0)
                        throw new Exception("Error reading file count");
                    for (int i = 0; i < fileCount; i++)
                    {
                        var efile = new EmbeddedFile();
                        efile.FileName = ReadStringFromStream(astream);
                        ...
                        while (ssize > 0)
                        {
                            ...
                            int readed2 = astream.Read(buf2, 0, Convert.ToInt32(toread));
                            if (readed2 == 0)
                            {
                                throw new Exception("Error reading embedded file");
                            }
                            efile.Stream.Write(buf2, 0, readed2);
                            ssize -= readed2;
                        }
    ...
    TextObjectStruct objt)
        {
            ...
            while (newextent.Y > maxextent.Y)
            {
                ...
                //sergi 01-08-2025: indico <= porqu� si el currentpos=0 no termina nunca
                if (currentpos <= 0)
                    break;
                objt.Text = originalstring.Substring(0, currentpos);
                newextent = maxextent;

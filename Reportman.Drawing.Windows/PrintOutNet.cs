            ...
            switch (obj.MetaType)
            {
                ...
                case MetaObjectType.Image:
                    ...
                    if ((obji.PreviewOnly) && (!Previewing))
                        return;
                    System.Drawing.Image abit = null;
                    MemoryStream astream = page.GetStream(obji);
                    try
                    {
                        abit = Image.FromStream(astream);
                    }
                    catch
                    {
                        // Catch image reading errors, image will not print
                        // a red rectangle will print instead
                    }
                    ...
                    if (abit == null)
                    {
                        graph.DrawRectangle(Pens.Red, destrec);
                    }
                    else
                    {
        ...
        protected void TextRectJustify(Graphics gr, Rectangle arect, TextObjectStruct atext, Font nfont, SolidBrush sbrush)
        {
            ...
            for (i = 0; i < linfos.Count; i++)
            {
                ...
                if (!dojustify)
                {
                    ...
                    StringFormat newFormat = (StringFormat)sformat.Clone();
                    newFormat.FormatFlags = sformat.FormatFlags | StringFormatFlags.NoClip;
                    if ((sformat.Alignment == StringAlignment.Far || sformat.LineAlignment == StringAlignment.Far) && (!atext.CutText))
                    {
                        SizeF textSize = gr.MeasureString(astring, nfont, new SizeF(float.MaxValue, float.MaxValue), newFormat);
                        if (textSize.Width > newWidth && sformat.Alignment == StringAlignment.Far)
                        {

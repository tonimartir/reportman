...
export abstract class MetaObject {
    ...
    public FillFromBuf(buf: ArrayBuffer, index: number): void {
			...
			this.MetaType = <MetaObjectType>StreamUtil.byteArrayToByte(buf,index + 16);
    }
...
{
    public CustomX:number;
    public CustomY:number;
...
export class ArrayBufferList
{
    ...
    {
        let result:ArrayBuffer = this.buffers[id];

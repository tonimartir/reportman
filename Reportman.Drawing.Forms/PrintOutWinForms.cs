    ...
    public class PrintOutWinForms : PrintOutPrint
    {
        ...
        public bool ExecutePrintDialog()
        {
            using (PrintDialog pdia = new PrintDialog())
            {
                ...
                pdia.Document = doc;
                DialogResult aresult = pdia.ShowDialog(MainForm);
                    ...
                    using (PrintOutText ntextdriver = new PrintOutText())
                    {
                        ...
                        if (PreparePrint(meta))
                        {
                            ...
                            if (doprint)
                            {
                                ...
                                byte[] atext = ntextdriver.PrintResultStream.ToArray();
						...
						throw new Exception("Feature PrintPreviewDialog not supported");
                            ...
                            if (SystemPreview)
                            {
                                using (PrintPreviewDialog previewdia = new PrintPreviewDialog())
                                {
                                    ...
                                    previewdia.ShowDialog(MainForm);
                                }
                            }
                            else
                            {
        ...
        public Bitmap DrawChartBitmap(Series nseries)
        {
            ...
            foreach (SeriesItem sitem in nseries.SeriesItems)
            {
                        ...
                        case ChartType.Area:
                            switch (nseries.MultiBar)
                            {
                                case BarType.Stacked:
                                    chartserie.ChartType = System.Windows.Forms.DataVisualization.Charting.SeriesChartType.StackedArea;
                                    break;
                                case BarType.Stacked100:
                                    chartserie.ChartType = System.Windows.Forms.DataVisualization.Charting.SeriesChartType.StackedArea100;
                                    break;
                                default:
                ...
                for (int j = 0; j < sitem.Values.Count; j++)
                {
                    ...
                    {
                        int idx = chartserie.Points.AddXY(j, sitem.Values[j]);
                        // int idx = chartserie.Points.AddXY(ncaption.Values[j]);

                        npoint = chartserie.Points[idx];
                        npoint.AxisLabel = ncaption;
                    }
                    if (afontsizeY != 0)
                    {
                        if (FontPunt == null)
                ...
                if (nseries.ShowLegend && (sitem.ChartStyle != ChartType.Pie))
                {
                    if (chartserie.Points.Count > 0)
                    {
                        System.Windows.Forms.DataVisualization.Charting.Legend nlegend;
                        if (nchart.Legends.Count == 0)
                        {
                            nlegend = new System.Windows.Forms.DataVisualization.Charting.Legend(idxserie.ToString());
                            nlegend.Font = new Font(nlegend.Font.FontFamily, afontsize);
                            nlegend.Docking = System.Windows.Forms.DataVisualization.Charting.Docking.Right;
                        ...
                        nlegend.CustomItems.Add(chartserie.Points[0].Color, sitem.Caption);*/
                    }
                }
                acolor = ((acolor + 1) % (MAX_SERIECOLORS));
                idxserie++;

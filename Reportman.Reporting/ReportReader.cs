...
namespace Reportman.Reporting
{
    ...
    public class ReportReader
    {
        ...
        {
            FileStream stream = new(afilename, System.IO.FileMode.Open,
        ...
        {
            StringBuilder aresult = new(propvalue.Length);
            int i = 0;
            ...
            for (int i = 0; i < areport.SubReports.Count; i++)
            {
                ...
                for (int j = 0; j < subrep.Sections.Count; j++)
                {
                    ...
                    if (sec.ChildSubReportName.Length > 0)
                    {
                        ...
                        if (sec.ChildSubReport.ParentSection != sec)
                        {
                            sec.ChildSubReport.ParentSection = sec;
                        }
                        if (sec.ChildSubReport.ParentSubReport != sec.ChildSubReport)
                        {
        ...
        private void ReadPropReport()
        {
            switch (propname)
            {
                ...
                case "LINESPERINCH":
                    ...
                    break;
                case "PDFCONFORMANCE":
                    areport.PDFConformance = (PDFConformanceType)GetAsInteger();
                    break;
                case "PDFCOMPRESSED":
                    areport.PDFCompressed = GetAsBool();
                    break;
                case "DOCAUTHOR":
                    areport.DocAuthor = GetAsString();
                    break;
                case "DOCCREATOR":
                    areport.DocCreator = GetAsString();
                    break;
                case "DOCPRODUCER":
                    areport.DocProducer = GetAsString();
                    break;
                case "DOCSUBJECT":
                    areport.DocSubject = GetAsString();
                    break;
                case "DOCTITLE":
                    areport.DocTitle = GetAsString();
                    break;
                case "DOCCREATIONDATE":
                    areport.DocCreationDate = GetAsString();
                    break;
                case "DOCMODIFICATIONDATE":
                    areport.DocModificationDate = GetAsString();
                    break;
                case "DOCKEYWORDS":
                    areport.DocKeywords = GetAsString();
                    break;
                case "DOCXMPCONTENT":
                    areport.DocXMPContent = GetAsString();
                    break;
                default:
                    if (propname[0] != '/')
        ...
        private void ReadCompProp(PrintPosItem comp)
        {
            ...
            switch (propname)
            {
                ...
                case "DOAFTERPRINT":
                    ...
                    break;
                case "ANNOTATION":
                    comp.AnnotationExpression = GetAsString();
                    assigned = true;
                    break;
                case "DOBEFOREPRINT":
    ...
    public class ReportWriter
    {
        ...
        private void WriteReportPropsXML(BaseReport areport, Stream astream, StreamVersion version)
        {
            ...
            if (areport.PDFConformance != PDFConformanceType.PDF_1_4)
                WritePropertyI("PDFCONFORMANCE", (int)areport.PDFConformance, astream);
            if (areport.PDFCompressed)
                WritePropertyBool("PDFCOMPRESSED", areport.PDFCompressed, astream);
            if (areport.DocAuthor != null && areport.DocAuthor.Length > 0)
                WritePropertyS("DOCAUTHOR", areport.DocAuthor, astream);
            if (areport.DocCreator != null && areport.DocCreator.Length > 0)
                WritePropertyS("DOCCREATOR", areport.DocCreator, astream);
            if (areport.DocProducer != null && areport.DocProducer.Length > 0)
                WritePropertyS("DOCPRODUCER", areport.DocProducer, astream);
            if (areport.DocSubject != null && areport.DocSubject.Length > 0)
                WritePropertyS("DOCSUBJECT", areport.DocSubject, astream);
            if (areport.DocTitle != null && areport.DocTitle.Length > 0)
                WritePropertyS("DOCTITLE", areport.DocTitle, astream);
            if (areport.DocCreationDate != null && areport.DocCreationDate.Length > 0)
                WritePropertyS("DOCCREATIONDATE", areport.DocCreationDate, astream);
            if (areport.DocModificationDate != null && areport.DocModificationDate.Length > 0)
                WritePropertyS("DOCMODIFICATIONDATE", areport.DocModificationDate, astream);
            if (areport.DocKeywords != null && areport.DocKeywords.Length > 0)
                WritePropertyS("DOCKEYWORDS", areport.DocKeywords, astream);
        ...
        private static void WriteComponentXML(PrintPosItem comp, Stream astream)
        {
            ...
            if (comp.AnnotationExpression != null && comp.AnnotationExpression.Length > 0)
                WritePropertyS("ANNOTATION", comp.AnnotationExpression, astream);
            // CommonPos
        ...
        {
            string nresult = "";
            using (MemoryStream astream = new())
            {
                StreamUtil.SWriteLine(astream, "<SECTION>");
        ...
        private void IntSaveToStream(Stream astream, StreamVersion version)
        {
            ...
            foreach (EmbeddedFile efile in areport.EmbeddedFiles)
            {
                astring = "<EMBEDDEDFILE>";
                StreamUtil.SWriteLine(astream, astring);
                WritePropertyS("FILENAME", efile.FileName, astream);
                WritePropertyS("MIMETYPE", efile.MimeType, astream);
                WritePropertyS("DESCRIPTION", efile.Description, astream);
                WritePropertyS("CREATIONDATE", efile.CreationDate, astream);
                WritePropertyS("MODIFICATIONDATE", efile.ModificationDate, astream);
                WritePropertyI("RELATIONSHIP", (int)efile.AFRelationShip, astream);
                efile.Stream.Seek(0, SeekOrigin.Begin);
                WritePropertyB("STREAM", efile.Stream, astream);
                astring = "</EMBEDDEDFILE>";
                StreamUtil.SWriteLine(astream, astring);
            }
            astring = "</REPORT>";
        ...
        {
            FileStream astream = new(afilename, System.IO.FileMode.Create,
